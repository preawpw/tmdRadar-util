...
function radar_callback(callback) {
    ...
    var imgmeta = require("./nck-aug2017.json");//ข้อมูลภาพเรดาร์น้ำฝน
    var smallimgmeta = imgmeta.slice(0, 4310);
    var bg = require("./images/bg-nck.json");//ภาพแบล็คกราว
    var fs = require('fs');
    ...
    Jimp.read(input2, function (err, img2) {
        async.eachSeries(smallimgmeta, function (item, next) {
            ...
            Jimp.read(input1, function (err, img1) {
                var jmp_img3 = new Jimp(img2.bitmap.width, img2.bitmap.height, black, function (err, img3) {
                    var w = img2.bitmap.width;//อ่านความกว้างภาพ
                    var h = img2.bitmap.width;//อ่านความยาวภาพ
                    try{
                    for (let y = 0; y <= h; y++) {
                        for (let x = 0; x <= w; x++) {
                            ...
                            var rgbCL2 = Jimp.intToRGBA(intCL2);//แปลงค่าสีจาก int เป็น rgb
                    ...
                    var sum = 0;
                    var n = 0;
                    var latitude = 13.74551;
                    var longitude = 100.53538;
                    var station = "E39";
                    var result;
            ...
            fs.writeFile('data2.csv', txtResult, (err) => {
                if (err) throw err;
            });
